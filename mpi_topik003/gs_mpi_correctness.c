  ...
  for (i = 0; i < rows; i++) {
    for (j = 0; j < columns; j++) {
      if (count == 0)
        ...
        printf("%lf \t", N[((n * i) + j)]); //row major accessing with blue.
      else if (count == 2)
        printf( "%lf \t", N[((n * i) + j)]); //row major accessing with Yellow.
      else
        printf("%lf \t", N[((n * i) + j)]); //row major accessing with Cyan.
