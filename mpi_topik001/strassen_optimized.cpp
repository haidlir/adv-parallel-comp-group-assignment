...
*/
  ...
  int main(int argc, char * * argv) {
    ...
    if (my_rank == 0) { //master process.
      for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++) {
          A(i, j) = 1.0; //As used in Cannon's simply for testing purposes
          B(i, j) = 2.0; // As used in Cannon's
  ...
  /* We completed sending the matrices to different processors */
    ...
    */
    ...
    if (my_rank == 0) {
      ...
      printf("Strassens computation time is %f seconds \n", computation_time);
      communication_time = strassens_time - computation_time;
      printf("Strassens communicaiton time is %f seconds \n\n", communication_time);
    }
